...
function DashboardApp() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans antialiased flex flex-col selection:bg-[#FF0066]/10 selection:text-[#FF0066] transition-colors duration-300">
      ...
      <nav id="navbar" className="bg-white dark:bg-slate-900 border-b border-slate-200/80 dark:border-slate-800 sticky top-0 z-40 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          
          {/* Left Block: Clean & Compact Breadcrumb */}
          <div className="flex items-center space-x-2.5 select-none shrink-0">
            <div className="w-2 h-6 bg-[#FF0066] rounded-xs"></div>
            <span className="font-sans font-black tracking-tight text-base sm:text-lg text-slate-900 dark:text-white flex items-center">
              COMPANY<span className="text-[#FF0066] font-normal ml-0.5">HERO</span>
            </span>
            <span className="hidden md:inline-block text-slate-300 dark:text-slate-700 text-xs">/</span>
            <span className="hidden md:inline-block text-xs font-medium text-slate-500 dark:text-slate-400">
              Operations & CX Intelligence
            </span>
          </div>

          {/* Right Block: Clock, Live Status, Lang, Theme, User */}
          <div className="flex items-center space-x-2 sm:space-x-3">
            <BrasiliaClock />

            {/* Subtle Live/Offline Status Indicator */}
            <button 
              onClick={handleReload}
              className="hidden sm:inline-flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/80 transition-colors cursor-pointer"
            ...
            <div className="relative" ref={langDropdownRef}>
              <button
                ...
                className="flex items-center space-x-1 px-2.5 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors cursor-pointer border border-transparent hover:border-slate-200 dark:hover:border-slate-700"
              ...
              {langMenuOpen && (
                <div className="absolute right-0 mt-1.5 w-32 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg py-1 z-50">
                  <button
                    onClick={() => { setLanguage('pt'); setLangMenuOpen(false); }}
                    className={`w-full px-3 py-1.5 text-left text-xs font-medium flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer ${language === 'pt' ? 'text-[#FF0066] font-bold' : 'text-slate-700 dark:text-slate-300'}`}
                  >
                    <span>Português</span>
                    {language === 'pt' && <Check className="w-3.5 h-3.5 text-[#FF0066]" />}
                  </button>
                  <button
                    onClick={() => { setLanguage('en'); setLangMenuOpen(false); }}
                    className={`w-full px-3 py-1.5 text-left text-xs font-medium flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer ${language === 'en' ? 'text-[#FF0066] font-bold' : 'text-slate-700 dark:text-slate-300'}`}
                  >
                    <span>English</span>
                    {language === 'en' && <Check className="w-3.5 h-3.5 text-[#FF0066]" />}
                  </button>
                  <button
                    onClick={() => { setLanguage('es'); setLangMenuOpen(false); }}
                    className={`w-full px-3 py-1.5 text-left text-xs font-medium flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer ${language === 'es' ? 'text-[#FF0066] font-bold' : 'text-slate-700 dark:text-slate-300'}`}
                  >
                    <span>Español</span>
                    {language === 'es' && <Check className="w-3.5 h-3.5 text-[#FF0066]" />}
                  </button>
                </div>
              )}
            </div>
            ...
            <button
              onClick={toggleTheme}
              className="p-2 bg-slate-100 dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-[#FF0066] rounded-xl transition-all cursor-pointer"
              title="Alternar tema"
            >
              {theme === 'light' ? <Moon className="w-3.5 h-3.5" /> : <Sun className="w-3.5 h-3.5" />}
            </button>

            {/* User Profile Trigger & Dropdown Popover */}
            {userProfile && (
              <div className="relative pl-1 border-l border-slate-200 dark:border-slate-800" ref={profileDropdownRef}>
                <button
                  onClick={() => {
                    setProfileMenuOpen(!profileMenuOpen);
                    setLangMenuOpen(false);
                  }}
                  className="flex items-center gap-2 p-1 pl-1.5 pr-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer"
                  title="Menu do perfil"
                >
                  {userProfile.photoURL ? (
                    <img src={userProfile.photoURL} alt="" className="w-7 h-7 rounded-full border border-slate-200 dark:border-slate-700 object-cover" />
                  ) : (
                  ...
                  <span className="hidden sm:inline-block text-xs font-semibold text-slate-800 dark:text-slate-200 max-w-[110px] truncate">
                    {userProfile.displayName?.split(' ')[0] || userProfile.displayName}
                  </span>
                  <ChevronDown className="w-3 h-3 text-slate-400" />
                </button>
                ...
                {/* Profile Dropdown */}
                {profileMenuOpen && (
                  <div className="absolute right-0 mt-1.5 w-64 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-3.5 z-50 space-y-3">
                    {/* User Info Header */}
                    <div className="flex items-start gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
                      {userProfile.photoURL ? (
                        <img src={userProfile.photoURL} alt="" className="w-10 h-10 rounded-full border border-slate-200 dark:border-slate-700 object-cover" />
                      ) : (
                        <div className="w-10 h-10 rounded-full bg-[#FF0066]/10 text-[#FF0066] font-black flex items-center justify-center text-sm shrink-0">
                          {userProfile.displayName?.charAt(0) || 'H'}
                        </div>
                      )}
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 break-all mt-0.5 leading-tight">
                          {userProfile.email}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      {userProfile.role === 'admin' && (
                        <button
                          ...
                          className="w-full flex items-center space-x-2 px-2.5 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl transition-colors cursor-pointer"
                        >
                      ...
                      <button
                        ...
                        className="w-full flex items-center space-x-2 px-2.5 py-2 text-xs font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-xl transition-colors cursor-pointer"
                      >
